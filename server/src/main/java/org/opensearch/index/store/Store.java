...
public class Store extends AbstractIndexShardComponent implements Closeable, RefCounted {
    ...
    /**
     * The engine directory, set when a {@link SidecarProtectedDirectory} wrapper is installed.
     * When non-null, {@link #engineDirectory()} returns this wrapper instead of the raw
     * {@link StoreDirectory}. This ensures all engine construction paths use the wrapped
     * directory that protects sidecar files from IndexWriter's garbage collection.
     */
    private volatile Directory engineDirectory;
    ...
    /**
     * Installs a {@link SidecarProtectedDirectory} wrapper so that subsequent calls to
     * {@link #engineDirectory()} return the wrapper instead of the raw {@link StoreDirectory}.
     * This must be called before engine creation to ensure IndexWriter uses the wrapped
     * directory that protects sidecar star tree files from garbage collection.
     *
     * @param wrapper the sidecar-protected directory wrapper
     */
    ...
    /**
     * Returns the directory that engine construction paths should use. If a
     * {@link SidecarProtectedDirectory} has been installed via
     * {@link #installSidecarDirectory(SidecarProtectedDirectory)}, returns that wrapper.
     * Otherwise, returns the raw {@link StoreDirectory} from {@link #directory()}.
     * <p>
     * All engine construction paths — including {@code resetEngineToGlobalCheckpoint()} and
     * {@code openEngineAndRecoverFromTranslog()} — should use this method instead of
     * {@link #directory()} to ensure sidecar files are protected from IndexWriter's GC.
     *
     * @return the engine directory (wrapped if sidecar protection is installed, raw otherwise)
     */
    ...
    public void cleanupAndVerify(String reason, MetadataSnapshot sourceMetadata) throws IOException {
        ...
        try (Lock writeLock = directory.obtainLock(IndexWriter.WRITE_LOCK_NAME)) {
            for (String existingFile : directory.listAll()) {
                ...
                // Skip sidecar-protected files — these are star tree sidecar files that are not tracked
                // in SegmentInfos but must be preserved. The SidecarProtectedDirectory wrapper protects
                // them from IndexWriter's GC, but cleanupAndVerify() operates on the raw StoreDirectory.
